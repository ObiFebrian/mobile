...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnRegister.setOnClickListener(view -> {
            if(editnama.getText().length()>0 && editemail.getText().length()>0 && editpassword.getText().length() >0 && editconfirmpassword.getText().length()>0) {
                if(editpassword.getText().toString().equals(editconfirmpassword.getText().toString())){
                register(editnama.getText().toString(),editemail.getText().toString(),editpassword.getText().toString());
                }else{
                    Toast.makeText(getApplicationContext(), "Silahkan masukkan paswword yang sama", Toast.LENGTH_SHORT).show();
                }
